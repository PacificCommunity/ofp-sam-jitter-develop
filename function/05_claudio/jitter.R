...
setMethod("jitter", signature(par="MFCLPar", sd="numeric", seed="numeric"),
  function(par, sd, seed){
    ...
    # Percent maturity
    # if converting maturity at length to age using a spline
    if(flagval(par,2,188)$value > 0){
    ...
    if(flagval(par,1,155)$value > 0){
      rec_orthogonal(par)[] <- rec_orthogonal(par) *
        rnorm(prod(dim(rec_orthogonal(par))),1,sd)
      new_orth_coffs(par)[] <- 0
      annual_rel_rec_coffs(par) <- annual_rel_rec_coffs(par) * 0
      orth_coffs(par) <- orth_coffs(par) *
        rnorm(prod(dim(orth_coffs(par))),1,sd)
    }
