...
setMethod("jitter", signature(par="MFCLPar",sd="numeric",seed="numeric"),function(par,sd,seed) {
 ...
 # }
  ...
  #orth_coffs(par) * rnorm(length(orth_coffs(par)),1,sd)
  #xyplot(data~year|area, group=qname, data=FLQuants(qts(rec_orthogonal(par)),
  #                                                  qts(rec_orthogonal(par)*rnorm(prod(dim(rec_orthogonal(par))),1,sd))), type="l")
  }
